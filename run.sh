...
# Config: GPU
# =========================
# PATCH: чистые URL/host без markdown
...
# PATCH: tail CPU log to console, converting \r -> \n
: > "$CPU_LOGFILE"
...
run_gpu() {
  while true; do
    ...
    # GPU стартуем как раньше, но без вывода в консоль (тихо)
...
run_cpu() {
  while true; do
    ...
    # PATCH: НЕ используем run_with_pty_if_possible (он гонит через awk).
    # Пишем "сырой" вывод в файл, а tail уже показывает его в консоль.
    stdbuf -oL -eL "$CPU_BIN" \
      --algorithm "$CPU_ALGO" \
      --pool "$CPU_POOL" \
      --wallet "$CPU_PUBKEY" \
