...
# Config: GPU
# =========================
# !!! ВАЖНО: убрал markdown [..](..). Должны быть чистые URL.
...
run_with_pty_if_possible() {
  ...
  if command -v script >/dev/null 2>&1; then
    # util-linux script: даёт PTY и возвращает exit code команды
    # -q: quiet, -e: return exitcode of child, -c: command, /dev/null: не писать файл
    script -q -e -c "$(printf '%q ' "$@")" /dev/null 2>&1 | prefix_logs "$tag"
    return ${PIPESTATUS[0]}
  fi

  # fallback: без PTY, но с line-buffered
...
run_cpu() {
  while true; do
    ...
    # Важно: у SRBMiner обычно worker задаётся через --worker, а не --password.
    # Если тебе реально нужен password — вернёшь, но worker должен быть отдельным.
