...
# Бинарник, который ожидаем в архиве (если имя другое — скрипт покажет список файлов и упадёт)
GPU_BIN_NAME="vecnoski-miner"
...
# Запуск команды в псевдо-TTY, чтобы майнеры печатали логи как в "реальном терминале"
# Плюс line-buffered вывод.
run_in_tty() {
  local cmd="$1"
  # script создаёт псевдо терминал. -q тихо, -e возвращает exit code команды, -c команда.
  # stdbuf для более живого вывода
  script -q -e -c "stdbuf -oL -eL bash -lc '$cmd'" /dev/null
}

prefix_logs() {
  local tag="$1"
  # Важно: fflush() чтобы не буферизовалось на стороне awk
...
GPU_BIN="$(find_executable "$GPU_WORKERDIR/extract" "$GPU_BIN_NAME" || true)"
if [[ -z "${GPU_BIN:-}" ]]; then
  log "❌ Не найден GPU бинарник '$GPU_BIN_NAME' в архиве. Список файлов:"
...
CPU_BIN="$(find_executable "$CPU_WORKERDIR/extract" "$CPU_BIN_NAME" || true)"
if [[ -z "${CPU_BIN:-}" ]]; then
  log "❌ Не найден CPU бинарник '$CPU_BIN_NAME' в архиве. Список файлов:"
...
run_gpu() {
  while true; do
    ...
    # Собираем команду как строку, чтобы прогнать через bash -lc внутри script (TTY)
    local cmd
    cmd="\"$GPU_BIN\" \
      --mining-address \"$GPU_PUBKEY\" \
      --stratum-server \"$GPU_STRATUM_SERVER\" \
      --stratum-port \"$GPU_STRATUM_PORT\" \
      --stratum-worker \"$WORKER_NAME\" 2>&1"

    set +e
    # Подаём вывод в awk с префиксом
    run_in_tty "$cmd" | prefix_logs "GPU"
...
run_cpu() {
  while true; do
    ...
    cmd="\"$CPU_BIN\" \
      ...
      --pool \"$CPU_POOL\" \
      --wallet \"$CPU_PUBKEY\" \
      --password \"$WORKER_NAME\" 2>&1"

    set +e
    run_in_tty "$cmd" | prefix_logs "CPU"
    EXIT_CODE=${PIPESTATUS[0]}
    set -e
